...
Документ сверяется с текущим `flows.json` и новой постоянной памятью `project-memory/`. Для быстрых последующих доработок краткие инварианты интеграций вынесены в `project-memory/technical-notes.md`; этот файл остается расширенным описанием протоколов, data-файлов и командных путей.

Подтверждено по текущему flow:

- production UI состоит из страниц `Угловая скорость`, `БУН/БЕП(Tilt/Gap)`, `Настройка`, `Журнал`, `Мониторинг`;
- рабочие `ui-template` (`Угловая скорость`, `Сценарии`, `БУН`, `BEP`, `Настройка`, `Журнал`, `Мониторинг`) имеют `passthru=false`;
- ELMO production-путь остается `CommandHandler -> ELMO XState (UDP) -> ResponseParser`;
- БУН и БЕП по-прежнему подключаются через MQTT-шлюзы, а не прямой UDP из Node-RED UI;
- remote-control `CommandGate` остается обязательным входным gate для рабочих Dashboard-команд.

Добавлено руководство оператора `docs/operator-manual.md`; оно описывает пользовательское поведение этих же интеграций без низкоуровневых деталей.
...
5. чтение `SR`, `MF`, `EE[5]`
6. `MO=1`
7. один оборот через абсолютное движение `PA=<текущий PX + ca18>` и `BG` при `SP=5°/с`, `AC=DC=2.5°/с²`
...
- ожидание скорости начинается после `CMD.ACKED` для `set_jp`, timeout считается от текущей скорости до целевой с учетом `AC/DC` и 10-секундного запаса;
...
3. `packages/nc3-elmo-machines/src/`
4. `project-memory/technical-notes.md` как краткую память по инвариантам
5. приложенные `docx` как справочную базу
