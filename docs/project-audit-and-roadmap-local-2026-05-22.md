...
## 14. Обновление после доработки протокола и настроек от 2026-05-25

### 14.1. Что изменено в `flows.json`

1. `ProtocolManager` исправлен для записи реальных переводов строк в файлах протокола. Заголовок больше не собирается через литерал `\\n`, поэтому в `C:\NC3\protocols\protocol_<timestamp>.txt` строки разделяются нормальными переносами.
2. В заголовок протокола добавлена строка колонок:

   `Значение_скорости Погрешность Файл_данных # градус/с %`

3. При записи с флагом `Запись исходных данных` строка результата теперь содержит имя data-файла:

   `10.000000  0.0000  data_<timestamp>.txt # градус/с  %  файл исходных данных`

   Вкладка `Журнал` распознает такую строку как ссылку и открывает связанный файл из `C:\NC3\data`.
4. Закрытие протокола при активной записи больше не теряет измерение: перед финальным маркером `=====` `ProtocolManager` пытается завершить текущую запись, записать строку результата и, если включен raw-data режим, создать файл исходных данных.
5. В global context добавлен активный флаг `is_recording_raw`. Он выставляется вместе с `is_recording` при `start_recording` и сбрасывается при `stop_recording`, `record_measurement` и `close_protocol`. Предыдущий `recording_save_raw_data` оставлен как пользовательское состояние флага UI.
6. `GlobalStateReader` теперь возвращает `is_recording_raw` в init-снимке, чтобы компоненты могли корректно синхронизироваться после `reread`.
7. Вкладка `Настройка` визуально разделена на блоки: основные настройки, сохранение угла, режим ввода угла, периоды опроса, диапазоны скорости, диапазон наклона, скорость переключения энкодеров, БЕП и текстовые калибровки. Модель данных и топик сохранения `settings_aply` не менялись.

### 14.2. Проверки

- `flows.json` успешно парсится как JSON.
- Все `function`-узлы проходят синтаксическую проверку через `new Function(...)`.
- Все Vue `<script>` блоки `ui-template` проходят синтаксическую проверку после преобразования поддержанного Dashboard 2 wrapper `export default` в `return`.
