...
- `angle_buffer` пишет точки `{ t: tm_us/1e6, tm_us, angle, position_ticks, resolution }`, пока `is_recording`, обрезая буфер по длительности текущей записи;
...
1. Persistent event journal (файл/БД) с уровнями событий вместо только `global.logs`.
2. Частичное восстановление выдержки сценария после паузы/аварии, если это будет подтверждено методикой испытаний.
3. Эксплуатационная синхронизация runtime-сценариев: явный backup/copy из `C:\NC3\scenarios` в репозиторий, если операторские правки должны попадать в git.
