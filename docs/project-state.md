...
- Ложный таймаут на шаге после смены пары головок (стенд: уставка 30 °/с, факт 9,24 °/с, «разгон 9,9 с»): расчёт разгона брал устаревшую «текущую скорость» 25 °/с прошлого шага (в низком разрешении буфер PX/TM пуст, выборок нет), хотя ось после homing стояла. Теперь: после homing/паузы текущая скорость = 0; значение старше 5 с считается нулём; готовность по окну TR[3]/TR[4] не ждёт окончания расчётного разгона; MS=0 засчитывается только при отсутствии VX (после ST привод даёт MS=0 при сохранённой уставке JV). Чипы «Готов» в UI — только по окну.
- Вкладки «Мониторинг»/«Угловая скорость»: снимок `scenario_state` в простое перезаписывал выбранный файл сценария именем последнего запуска (каждый reread) — выбор синхронизируется только с работающим/приостановленным сценарием.

- Прогресс поиска метки «стоял на 1 %»: `EventLogService` при каждом `event_log_refresh` (монтирование вкладок «Угловая скорость»/«Журнал») пересобирал `global.drive_init_state` из журнала — с `in_progress=false` и без полей прогресса/захвата метки; `DriveInitState` переставал накапливать угол, а сообщение о сбросе на «Мониторинг» не приходило. Теперь журнал используется только как резерв, пока в global нет статуса (после перезапуска); `DriveInitState` сохраняет служебные поля прогресса и без изменения видимого состояния.
