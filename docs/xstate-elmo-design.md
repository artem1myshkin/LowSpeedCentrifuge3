...
## Актуализация 2026-06-11: текущее поведение сценарного ожидания

Таймаут достижения скорости в сценариях теперь считается от текущей фактической скорости до целевой. `scenario.js` учитывает `AC`, `DC`, торможение, разгон в том же направлении и смену направления через ноль; прежняя формула `abs(targetSpeed)/AC + 10 с` остается только частным случаем старта из нулевой скорости. Сам транспортный контракт при этом не изменился: сценарии по-прежнему используют путь `CommandHandler -> ELMO XState (UDP) -> ResponseParser`.

Корневая папка `project-memory/` добавлена как краткая постоянная память проекта: она фиксирует текущие runtime-пути, ограничения Node-RED Dashboard 2, remote-control, сценариев, протоколов и аппаратных шлюзов.
...
Timeout ожидания скорости считается не от старта сценарного шага, а от ACK команды `set_jp`. В текущем коде расчет идет от текущей скорости до целевой с учетом `AC/DC` и 10-секундного запаса; время смены разрешения и `Drive Init` в этот timeout не входит.
...
Критерий готовности шага вынесен из сценарного файла в настройки ПО: `settings.general.speedReadyTolerancePercent` (0..100 %) плюс время устойчивости `advanced.speedStableTimeMs`. Timeout текущей реализации стартует после ACK `set_jp` и вычисляется от текущей скорости до целевой с учетом `AC/DC`; `advanced.speedReachTimeoutMs` остался fallback-настройкой evaluator-а.
