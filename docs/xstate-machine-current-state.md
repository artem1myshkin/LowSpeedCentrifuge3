...
### Компенсация Windows-таймера

`setTimeout(N)` на Windows округляется ВВЕРХ до ближайшего тика системного таймера (~15.625 мс). При цели 30 Гц «честный» запрос 18 мс лежит на втором тике ≈ 31 мс, и цикл получается ~46 мс → ~22 Гц (это и наблюдалось в первом замере).

Параметр `pollOptions.timerCompensationMs` (Node-RED-обвязка задаёт `8` мс) вычитается из `POLL_DELAY` перед отправкой в `setTimeout`. Запрос 18 → 10 мс < тика → лежит на ПЕРВОМ тике ≈ 15.6 мс, цикл ~30.6 мс → **~32 Гц** при цели 30. Один сэкономленный тик за цикл. Дальше — плато: без `timeBeginPeriod(1)` на хосте быстрее не уехать.

На Linux/c включённым `timeBeginPeriod(1)` таймер уже точный — компенсация 8 мс там даст overshoot (~+25% к фактической). Поэтому дефолт пакета — `0`, а Windows-значение `8` задано только в Node-RED-обвязке этого проекта.
