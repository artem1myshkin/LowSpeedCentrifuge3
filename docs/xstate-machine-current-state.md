...
Транспорт переведён на **UDP**. ELMO слушает команды на `192.168.1.2:5001`, ответы приходят на локальный порт `5005` (`udp out` / `udp in`). Ключевое отличие от TCP-схемы: **один логический poll = одна датаграмма = один ответ**. Весь набор полей батчится в одну командную строку (`TM;PX;VX;`), и ELMO отвечает одной датаграммой, которая уже является готовым кадром. Поэтому **`FrameSplitter` и idle-gap больше не нужны** — именно они ограничивали TCP-путь до ~4 Гц.

Почему это снимает потолок:

- в TCP `sit` один логический poll слался как несколько отдельных read-команд, и `FrameSplitter` ждал `80 ms` тишины после каждой → потолок `~4 Hz` для `VX/PX/TM`;
- в UDP логический poll — это одна датаграмма с батчем, и одна датаграмма ответа: нет ожидания тишины, нет посимвольной пересборки.

Замер стендового теста (inject `TM;` @ `repeat 0.033`, по часам ELMO `TM`): фактически **~24.7 Гц** (средний интервал ~40.4 мс, разброс 34–48 мс, без потерь пакетов). Узкое место — **таймер Node-RED/Windows `inject`/`setInterval` (гранулярность ~15.6 мс)**, а не ELMO и не UDP. ELMO по UDP держит ≥25–30 Гц и отвечает практически мгновенно.

Что осталось подтвердить на стенде:

1. ELMO стабильно отвечает на батч `TM;PX;VX;` (и на `full_state`-батч) **одной** датаграммой со всеми полями. Если ответ дробится на несколько датаграмм, транспорт это покажет событием `POLL.BAD_FRAME` (не хватает полей), и тогда нужно либо упростить батч, либо собирать кадр по числу полей.
2. Точный ровный 30 Гц требует более точного источника тиков, чем `inject` (например, самотактируемый `setTimeout` транспорта уже это делает; голый `inject` всегда джиттерит на Windows).
...
- батчит логический poll в одну командную датаграмму;
...
3. По UDP логический poll батчится в одну датаграмму, ответ — одна датаграмма (готовый кадр). Idle-gap и `FrameSplitter` убраны.
...
6. `PollRateMeter` показывает валидные логические frames/sec.
7. Замер inject-теста по UDP: ~24.7 Гц, ограничение — таймер `inject`/`setInterval` на Windows (~15.6 мс), не ELMO и не UDP. Потерь датаграмм не наблюдалось.
8. Если ELMO дробит ответ на батч на несколько датаграмм, это видно по `POLL.BAD_FRAME` (нехватка полей) — тогда упростить батч или собирать кадр по числу ожидаемых полей.
