...
Текущее состояние XStateMachine, fast raw polling, ограничения частоты, результаты отладки TCP и инструкция после `git pull` вынесены в `docs/xstate-machine-current-state.md`. Этот файл остается подробным историческим описанием архитектуры и может содержать старые примеры до последних изменений poll-алгоритма.
